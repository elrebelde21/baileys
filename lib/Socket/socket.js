...
export const makeSocket = (config) => {
    ...
    if (printQRInTerminal) {
        logger.warn({}, '⚠️ La opción `printQRInTerminal` ha quedado obsoleta. Ya no recibirás códigos QR en la terminal automáticamente. Por favor, escucha el evento `connection.update` tú mismo y gestiona el QR a tu manera. Puedes eliminar este mensaje quitando esa opción. Este mensaje será eliminado en una versión futura');
    }
    if (browser[1].toLocaleLowerCase().includes('android')) {
        logger.warn('⚠️ Usar el navegador de Android es experimental y puede provocar un comportamiento inesperado. Úsalo bajo tu propio riesgo.');
    }
    ...
    if (syncDisabled) {
        logger.warn('⚠️ PELIGRO: DESACTIVAR TODA LA SINCRONIZACIÓN CON `shouldSyncHistoryMsg` IMPIDE QUE BAILEYS ACCEDA A LOS MAPEOS INICIALES DE LID, LO QUE PROVOCA INESTABILIDAD Y ERRORES DE SESIÓN');
    }
    ...
    const defaultCode = `MITZUKI${randomDigit}`;
    
    // Usar código personalizado si se proporciona, de lo contrario usar el predeterminado
    const pairingCode = customPairingCode || defaultCode;
