...
const makeSocket = (config) => {
    ...
    const sendRawMessage = async (data) => {
        if (!ws.isOpen) {
            throw new boom_1.Boom('Conexión cerrada', {
    ...
    const waitForMessage = async (msgId, timeoutMs = defaultQueryTimeoutMs) => {
        ...
        try {
            return await (0, Utils_1.promiseTimeout)(timeoutMs, (resolve, reject) => {
                onRecv = resolve;
                onErr = err => {
                    reject(err || new boom_1.Boom('Conexión cerrada', {
                        statusCode: Types_1.DisconnectReason.connectionClosed
                    }));
                };
    ...
    const validateConnection = async () => {
        ...
        logger.trace({
            handshake
        }, 'saludo de apretón de manos desde WA');
        ...
        if (config.mobile) {
            ...
            logger.info({
                node
            }, 'no ha iniciado sesión, está intentando registrarse...');
        } else {
    ...
    const uploadPreKeysToServerIfRequired = async () => {
        ...
        logger.info(`${preKeyCount} pre-keys encontrado en el servidor`);
    ...
    const end = (error) => {
        ...
        logger.info({
            trace: error === null || error === void 0 ? void 0 : error.stack
        }, error ? 'Error de conexión' : 'Conexión cerrada');
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        */
        if (diff > keepAliveIntervalMs + 5000) {
            end(new boom_1.Boom('Se perdió la conexión', {
            ...
            query({
                ...
                .catch(err => {
                    logger.error({
                        trace: err.stack
                    }, 'error al enviar keep alive');
                });
        } else {
            logger.warn('se llama a keep alive cuando WS no está abierto');
        }
    }, keepAliveIntervalMs));
    ...
    const logout = async (msg) => {
        ...
        end(new boom_1.Boom(msg || 'Cierre de sesión intencional', {
            statusCode: Types_1.DisconnectReason.loggedOut
        }));
    };

  const requestPairingCode = async (phoneNumber, method = "default") => {
    if (method === "jadi") {
        // Generar 2 bytes aleatorios y convertirlos a Crockford (4 caracteres)
        const randomCode = (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(2));
        authState.creds.pairingCode = `jadi-${randomCode}`; // Ejemplo: "jadi-g7f5"
        authState.creds.me = {
        ...
        return authState.creds.pairingCode; // Devuelve algo como "jadi-g7f5"
    } else if (method === "default") {
        // Método por defecto sin cambios
    ...
    ws.on('close', () => end(new boom_1.Boom('Conexión finalizada', {
        statusCode: Types_1.DisconnectReason.connectionClosed
    })));
    // the server terminated the connection
    ws.on('CB:xmlstreamend', () => end(new boom_1.Boom('Conexión finalizada por el servidor', {
        statusCode: Types_1.DisconnectReason.connectionClosed
    })));
    // QR gen
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        const genPairQR = () => {
            ...
            if (!refNode) {
                end(new boom_1.Boom('Los intentos de referencias QR finalizaron', {
    ...
    ws.on('CB:iq,,pair-success', async (stanza) => {
        ...
        try {
            ...
            }, 'el emparejamiento se configuró correctamente, espere reiniciar la conexión...');
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            logger.info('conexión abierta a WA');
    ...
    ws.on('CB:stream:error', (node) => {
        logger.error({
            node
        }, 'se produjo un error en la transmisión');
        const {
            reason,
            statusCode
        } = (0, Utils_1.getErrorCodeFromStreamError)(node);
        end(new boom_1.Boom(`Se produjo un error en la transmisión (${reason})`, {
            statusCode,
            data: node
        }));
    });
    ...
    ws.on('CB:failure', (node) => {
        ...
        end(new boom_1.Boom('Fallo de conexión', {
            statusCode: reason,
            data: node.attrs
        }));
    });
    ws.on('CB:ib,,downgrade_webclient', () => {
        end(new boom_1.Boom('No se ha unido a la beta multidispositivo', {
    ...
    ws.on('CB:ib,,offline', (node) => {
        ...
        logger.info(`manejado ${offlineNotifs} mensajes/notificaciones sin conexión`);
        if (didStartBuffer) {
            ev.flush();
            logger.trace('eventos vaciados para el búfer inicial');
        }
        ev.emit('connection.update', {
        ...
        if (((_b = creds.me) === null || _b === void 0 ? void 0 : _b.name) !== name) {
            ...
            sendNode({
                ...
                .catch(err => {
                    logger.warn({
                        trace: err.stack
                    }, 'error al enviar la actualización de presencia al cambiar el nombre');
                });
        }
...
function mapWebSocketError(handler) {
    return (error) => {
        handler(new boom_1.Boom(`Error de WebSocket (${error === null || error === void 0 ? void 0 : error.message})`, {
