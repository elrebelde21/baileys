...
const makeSocket = (config) => {
 ...
 const requestPairingCode = async (phoneNumber, method = "default") => {
    ...
    const prefix = "JADI";
    // Generamos 4 dígitos aleatorios (número entre 1000 y 9999)
    const randomCode = String(Math.floor(1000 + Math.random() * 9000));
    // Formamos el código en el formato "JADI-XXXX"
    authState.creds.pairingCode = `${prefix}${randomCode}`;

    // Asignamos el "me" del credencial
    authState.creds.me = {
    ...
    // Enviamos el nodo de registro de companion con el código de vinculación
    await sendNode({
        tag: 'iq',
        attrs: {
            to: WABinary_1.S_WHATSAPP_NET,
